...
function WishlistPage() {
    ...
    const fetchWishlist = async () => {
        try {
            const response = await axios.get(`${BASE_URL}accounts/wishlist/`, {
    ...
    const removeFromWishlist = async (hotelId) => {
        try {
            await axios.delete(
                `${BASE_URL}accounts/wishlist/${hotelId}/`,
                { headers: { Authorization: `Bearer ${authTokens.access}` } }
            );
            setWishlist((prev) =>
    ...
    return (
        <div style={styles.container}>
            <h1 style={styles.heading}>❤️ My Wishlist</h1>
                        ...
                        <div key={item.id} style={styles.card}>
                            ...
                            <div style={styles.buttons}>
                                ...
                                <button
                                    ...
                                    onClick={() => removeFromWishlist(item.hotel.id)}
                                >
                                    Remove
                                </button>
                            </div>
